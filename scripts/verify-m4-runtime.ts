...
import { buildTimelineWindow, initialFutureDates, requiredFutureCount } from "../src/timeline/Timeline";
...
const windowEntries = buildTimelineWindow({
  ...
  futureCount: 3,
  contentDays: rows,
  insertedKeys: new Set(["2026-06-15"]),
  pastLimit: 14,
});

assert(windowEntries[0].key === "2026-07-06", "timeline must place farthest future placeholder first");
assert(windowEntries[2].key === "2026-07-04", "timeline must place tomorrow immediately above today");
assert(windowEntries[3].key === "2026-07-03" && windowEntries[3].kind === "today", "today must sit after future placeholders");
assert(windowEntries.some((entry) => entry.key === "2026-06-15" && entry.kind === "placeholder"), "jumped empty past dates must render as placeholders");
assert(requiredFutureCount(today, addDays(today, 9)) === 9, "future jump sizing must match day distance");
assert(initialFutureDates(today).join(",") === "2026-07-06,2026-07-05,2026-07-04", "initial future dates must be descending");
